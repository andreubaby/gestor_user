...
@section('title', 'Editar: ' . $sequence->name)
...
<div class="min-h-screen text-slate-900">
    <main class="max-w-6xl mx-auto px-5 py-6">
        <x-ui.section-heading :title="'Editar: ' . $sequence->name" subtitle="Ajusta condiciones, pasos y adjuntos con la misma guía visual del panel." />
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2 text-xs">
            ...
            <a href="#section-danger" class="px-3 py-1 rounded-lg bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 transition">Zona peligro</a>
        </div>

        <form method="POST" action="{{ route('automation.sequences.update', $sequence) }}" class="space-y-5">
            @csrf
            @method('PUT')

            <!-- Información Básica -->
            <section id="section-basic" class="gradient-card rounded-2xl p-6 card-hover animate-slide-in">
                ...
                <div class="grid grid-cols-1 gap-6">
                    @if(($templates ?? collect())->isNotEmpty())
                        <div class="rounded-lg border border-blue-200 bg-blue-50 p-3">
                            <label class="block text-xs font-semibold uppercase text-blue-700 mb-2">Reemplazar pasos desde plantilla</label>
                            <div class="flex gap-2">
                                ...
                                <button type="button" id="apply-template-btn" class="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition">Aplicar</button>
                            </div>
                        </div>
                    @endif

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Nombre de la Secuencia *</label>
                        <input type="text" name="name" value="{{ old('name', $sequence->name) }}" required class="w-full px-4 py-3 rounded-lg border border-slate-300 bg-white input-focus outline-none">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Descripción</label>
                        <textarea name="description" rows="2" class="w-full px-4 py-3 rounded-lg border border-slate-300 bg-white input-focus outline-none resize-none">{{ old('description', $sequence->description) }}</textarea>
                    </div>
            ...
            <section id="section-steps" class="animate-slide-in" style="animation-delay: 0.1s">
                <div class="gradient-card rounded-2xl p-6 card-hover">
                    <div class="flex items-center gap-3 mb-6">
                        <div class="w-10 h-10 gradient-primary rounded-lg flex items-center justify-center text-white">
                    ...
                    <div id="steps-container" class="space-y-3 max-h-[62vh] overflow-y-auto pr-1 smooth-scroll"></div>
                    <input type="hidden" id="steps-input" name="actions" value="{{ old('actions', json_encode($sequence->actions ?? [])) }}">
                </div>
            </section>
        ...
        <!-- Zona Peligro -->
        <section id="section-danger" class="border-2 border-red-500/30 rounded-2xl p-6 bg-red-500/5 animate-slide-in mt-5" style="animation-delay: 0.3s">
            <div class="flex items-start gap-4">
                <svg class="w-6 h-6 text-red-600 flex-shrink-0 mt-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                <div class="flex-1">
                    <h3 class="title-font text-lg font-bold text-red-900 mb-2">Zona de Peligro</h3>
                    <p class="text-sm text-red-800 mb-4">Al eliminar esta secuencia, se perderán todas sus programaciones automáticas.</p>
                    <form method="POST" action="{{ route('automation.sequences.destroy', $sequence) }}" onsubmit="return confirm('¿Estás seguro? Esta acción es irreversible.');" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-6 py-2 gradient-danger rounded-lg text-white font-semibold btn-hover shadow-lg flex items-center gap-2">
...
<template id="step-template">
    <div class="step-item rounded-2xl p-4 card-hover group">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    @php
        $templatesPayload = ($templates ?? collect())
            ->keyBy('id')
    ...
    function restoreDraft() {
        try {
            ...
            const shouldRestore = window.confirm('Se encontró un borrador local de esta edición. ¿Restaurar?');
    ...
    try {
        const old = {!! json_encode(old('actions', $sequence->actions ?? [])) !!};

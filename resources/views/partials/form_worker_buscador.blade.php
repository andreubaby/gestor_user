<form method="POST" action="{{ route('buscador.worker.update', ['id' => $workerBuscador->id]) }}">
    @csrf
    @method('PUT')
    ...
    <div class="mb-4">
        ...
        <input type="text" id="buscador_worker_nombre" name="nombre"
               value="{{ old('nombre', $workerBuscador->name) }}"
    ...
    <div class="mb-4">
        ...
        <input type="email" id="buscador_worker_email" name="email"
               value="{{ old('email', $workerBuscador->email) }}"
               class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-blue-300">
    </div>
    ...
    <div class="text-right">
        <button type="submit" class="bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-800 transition">
            Guardar cambios
        </button>
    </div>
</form>
